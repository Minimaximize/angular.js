...
var getSnapshotVersion = function() {
  ...
  if (!version.prerelease || !version.prerelease.length) {
    ...
    // ZAP: not incrementing because we're just patching release versions
    //version.patch++;
  }
  // ZAP: not setting prerelease because we're only patching release versions
  //version.prerelease = jenkinsBuild ? ['build', jenkinsBuild] : ['local'];
  version.build = 'ZAP.' + getBuild(); // ZAP obviously the ZAP part is just us
  version.codeName = 'eventful-proposal';
